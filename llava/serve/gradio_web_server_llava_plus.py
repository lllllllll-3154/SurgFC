...
headers = {"User-Agent": "VS-Assistant"}
...
def load_demo_refresh_model_list(request: gr.Request):
    ...
    for i in models:
        if "llava" in i:
            model_filter.append(i)
    state = default_conversation.copy()
    return (state, gr.Dropdown.update(
        choices=model_filter,
        value=model_filter[0] if len(models) > 0 else ""),
        gr.Chatbot.update(visible=True),
        gr.Textbox.update(visible=True),
        gr.Button.update(visible=True),
        gr.Row.update(visible=True),
...
def http_bot(state, model_selector, temperature, top_p, max_new_tokens, with_debug_parameter_from_state, request: gr.Request):
    ...
    try:
        ...
        #debug only
        #print("here is the full text: ",response)
        # import ipdb; ipdb.set_trace()
    ...
    print("state_message: = = = = = ",state.messages)
    ...
    if tool_cfg is not None and len(tool_cfg) > 0:
        ...
        iou_sort_masks = None
        if 'boxes' in tool_response and tool_response['boxes'] is not None:
            tool_response['boxes'] = [[R(_b) for _b in bb]
                                      for bb in tool_response['boxes']]
        else:
            tool_response['boxes'] = []
        if 'logits' in tool_response and tool_response['logits'] is not None:
            tool_response['logits'] = [R(_l) for _l in tool_response['logits']]
        else:
            tool_response['logits'] = []
        if 'scores' in tool_response and tool_response['scores'] is not None:
            tool_response['scores'] = [R(_s) for _s in tool_response['scores']]
        else:
            tool_response['scores'] = []
...
def build_demo(embed_mode):
    ...
    with gr.Blocks(title="VS-Assistant", theme=gr.themes.Base()) as demo:
        state = gr.State()
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Accordion("Parameters", open=False, visible=False) as parameter_row:
                    ...
                    temperature = gr.Slider(
                        minimum=0.0, maximum=1.0, value=0.0, step=0.1, interactive=True, label="Temperature",)
                    top_p = gr.Slider(
            ...
            with gr.Column(scale=6):
                chatbot = gr.Chatbot(
                    elem_id="chatbot", label="VS-Assistant Chatbot", height=550)
                with gr.Row():
        ...
        with gr.Row():
            with gr.Column():
                gr.Examples(examples=[
                    [f"{cur_dir}/examples/test1.jpg",
                        "Can you detect the surgical instrument in the image."], 
                ], inputs=[imagebox, textbox], label="Detection Examples: ")
                gr.Examples(examples=[
                    [f"{cur_dir}/examples/test2.jpg",
                        "Can you segment the surgical instruments in the image."],
                ], inputs=[imagebox, textbox], label="Segmentation Examples: ")
                gr.Examples(examples=[
                    [f"{cur_dir}/examples/test6.jpg",
                        "What is this image?"],
                        [f"{cur_dir}/examples/0325_currte.jpg",
                        "Can you describe this image?"],
                        [f"{cur_dir}/examples/0325_cuttingforcep_sunvtion_tube.jpg",
                        "tell me about this image?"],
                ], inputs=[imagebox, textbox], label="Caption")
